...
static unsigned int MAX_NUM_BUF = 200000;
...
static long ximea_cam_pcie_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	...
	case XIMEA_CAM_PCIE_IOCLOCKMEM:
		...
		if(vma && vma->vm_start <= (unsigned long)lockmem.buffer && vma->vm_flags & VM_WRITE) { //buffer points to a valid CPU memory location
