...
#if GST_CHECK_VERSION(1,0,0)
			gst_buffer_insert_memory(buffer, -1, gst_memory_new_wrapped(GST_MEMORY_FLAG_READONLY, (guint8*)image.bp, image.width*image.height*(image.frm == XI_RAW8 ? 1 : 4), 0, image.width*image.height*(image.frm == XI_RAW8 ? 1 : 4), 0, 0));
#else
			gst_buffer_set_data(buffer, (guint8*)image.bp, image.width*image.height*(image.frm == XI_RAW8 ? 1 : 4));
#endif
